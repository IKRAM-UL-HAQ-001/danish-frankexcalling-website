...
class DemoSendExport implements FromQuery, WithHeadings, WithStyles, WithColumnWidths, WithMapping
{
    use Exportable;
    ...
    public function map($demo_send): array
    {

        return [
            $demo_send->id,
            $this->decryptData($demo_send->exchange_name),
            $this->decryptData($demo_send->user_name),
            $this->decryptData($demo_send->name),
            $this->decryptData($demo_send->phone->phone_number),
            $this->decryptData($demo_send->feedback),
            $this->decryptData($demo_send->amount),
            $demo_send->created_at,
            $demo_send->updated_at,
        ];
    }
