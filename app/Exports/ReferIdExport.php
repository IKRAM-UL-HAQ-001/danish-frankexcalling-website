...
class ReferIdExport implements FromQuery, WithHeadings, WithStyles, WithColumnWidths, WithMapping
{
    use Exportable;
    ...
    public function map($referId): array
    {

        return [
            $referId->id,
            $this->decryptData($referId->exchange_name),
            $this->decryptData($referId->user_name),
            $this->decryptData($referId->name),
            $this->decryptData($referId->phone->phone_number),
            $this->decryptData($referId->feedback),
            $this->decryptData($referId->amount),
            $referId->created_at,
            $referId->updated_at,
        ];
    }
