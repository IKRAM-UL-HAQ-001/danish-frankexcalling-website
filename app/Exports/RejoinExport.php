...
class RejoinExport implements FromQuery, WithHeadings, WithStyles, WithColumnWidths, WithMapping
{
    use Exportable;
    ...
    public function map($rejoin): array
    {
        return [
            $rejoin->id,
            $this->decryptData($rejoin->exchange_name),
            $this->decryptData($rejoin->user_name),
            $this->decryptData($rejoin->name),
            $this->decryptData($rejoin->phone->phone_number),
            $this->decryptData($rejoin->feedback),
            $this->decryptData($rejoin->amount),
            $rejoin->created_at,
            $rejoin->updated_at,
        ];
    }
