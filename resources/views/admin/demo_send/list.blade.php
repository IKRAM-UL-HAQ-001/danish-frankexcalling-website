                        ...
                        <div class="table-responsive p-0">
                            <table  class="table align-items-center mb-0 table-striped table-hover px-2">
                                ...
                                <tbody id="DataTableBody">
                                    @foreach ($DemoSends as $DemoSend)
                                        <tr data-user-id="a" data-exchange-id="a">
                                            <td style="width: 45%;" class="encrypted-data">{{ $DemoSend->user->name }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $DemoSend->exchange->name }}
                                            </td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $DemoSend->name }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $DemoSend->phone->phone_number }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $DemoSend->feedback }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $DemoSend->amount }}</td>
                                            <td>{{ $DemoSend->updated_at }}</td>
                                            <td style="width: 10%; text-align: center;">
                                                <div class="d-flex justify-content-center ">
                                                    ...
                                                    <button type="button" class="btn btn-info btn-sm edit-button mx-3"
                                                        data-id="{{ $DemoSend->id }}" data-name="{{ $DemoSend->name }}"
                                                        data-phone="{{ $DemoSend->phone->phone_number }}"
                                                        data-feedback="{{ $DemoSend->feedback }}"
                                                        data-amount="{{ $DemoSend->amount }}">
                                                        Edit
                                                    </button>
                                                </div>
                                            </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header d-flex justify-content-between align-items-center " style="background:#acc301;">
                    <h5 class="modal-title" id="exportModalLabel" style="color:white">Export Demo Sends</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">

                    <form id="reportForm" action="{{ route('export.demoSend') }}" method="POST">
                        @csrf
                        <div class="mb-3">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.getElementById('editDemoSendForm').addEventListener('submit', function(event) {
                ...
                // formData.append('phone_id', encryptData(form.editPhone.value));
                ...
                formData.append('task_name', 'demosend');
