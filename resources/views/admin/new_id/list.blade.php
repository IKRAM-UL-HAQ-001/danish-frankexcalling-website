                                ...
                                <tbody id="DataTableBody">
                                    @foreach ($NewIds as $NewId)
                                        <tr data-user-id="a" data-exchange-id="a">
                                            <td style="width: 45%;" class="encrypted-data">{{ $NewId->user->name }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $NewId->exchange->name }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $NewId->name }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $NewId->phone->phone_number ?? null }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $NewId->feedback }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $NewId->amount }}</td>
                                            <td >{{ $NewId->updated_at}}</td>
                                            <td style="width: 10%; text-align: center;">
                                                <div class="d-flex justify-content-center ">
                                                    <form method="POST" action="{{ route('admin.New_id.delete') }}">
                                                        @csrf
                                                    ...
                                                    <button type="button" class="btn btn-info btn-sm edit-button mx-3"
                                                        data-id="{{ $NewId->id }}" data-name="{{ $NewId->name }}"
                                                        data-phone="{{ $NewId->phone->phone_number ?? null }}"
                                                        data-feedback="{{ $NewId->feedback }}"
                                                        data-amount="{{ $NewId->amount }}">
                                                        Edit
                                                    </button>
                                                </div>
                                            </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header d-flex justify-content-between align-items-center " style="background:#acc301;">
                    <h5 class="modal-title" id="exportModalLabel" style="color:white">Export New Ids</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">

                    <form id="reportForm" action="{{ route('export.newId') }}" method="POST">
                        @csrf
                        <div class="mb-3">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-white" id="editDemoSendModalLabel">Edit New Id Data</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.getElementById('editDemoSendForm').addEventListener('submit', function(event) {
                ...
                formData.append('task_name', 'newid');
