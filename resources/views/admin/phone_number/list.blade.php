...
@section('content')
        ...
        <div class="row">
            <div class="col-11 mb-xl-0 mx-auto my-5 border w-full bg-white rounded d-flex flex-column">
                <div class="d-flex justify-content-between align-items-center p-3 border-bottom mb-5">
                    ...
                    <div>
                        <form action="{{ route('export.activePhoneNumbers') }}" method="POST" style="display: inline;">
                            @csrf
                            <button type="submit" class="btn btn-secondary ms-2" style="background: #344767;">Export Active
                                Numbers</button>
                        </form>
                    ...
                    <div class="card-body px-0 pb-2 px-3 col-12">
                        ...
                        <div class="table-responsive p-0">
                            <table id="" class="table align-items-center mb-0 table-striped table-hover px-2">
                                <thead>
                                    <tr>
                                        <th class="text-uppercase text-secondary font-weight-bolder text-dark">
                                            <input type="checkbox" id="checkAll" class="btn btn-primary mt-2"> Select
                                            All
                                        </th>
                                        ...
                                        <th class="text-uppercase text-secondary font-weight-bolder text-dark ps-2">User
                                            Name</th>
                                        <th class="text-uppercase text-secondary font-weight-bolder text-dark ps-2">Status
                                        </th>
                                    ...
                                    @foreach ($PhoneNumbers as $phoneNumber)
                                        <tr>
                                            ...
                                            <td style="width: 15%; text-align: center;">
                                                <form method="POST" action="{{ route('admin.phone_number.delete') }}">
                                                    @csrf
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form id="form" name="formm" method="post" action="{{ route('admin.phone_number.formPost') }}"
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select px-3" id="editExchange" name="user_id" required>
                                ...
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" class="encrypted-data">{{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="uploadForm" name="uploadFormm" method="post"
                        action="{{ route('admin.phone_number.filePost') }}" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3">
    ...
    <script defer>
        ...
        $('#searchFormPhone').on('submit', function(event) {
            ...
            $.ajax({
                url: "{{ route('admin.phone_number.search') }}", // Laravel route
                method: 'POST',
                data: {
        ...
        $(document).ready(function() {
            ...
            $('#deleteSelected').on('click', function() {
                ...
                $.ajax({
                    url: "{{ route('admin.phone_number.custom_delete') }}",
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                        ids: selectedIds
                    ...
                    success: function(response) {
                        ...
                        $('.select-checkbox').prop('checked', false).closest('tr').removeClass(
                            'table-active');
                        location.reload();
                    },
                    error: function(xhr) {}
            ...
            async function fetchAndEncryptExistingFile(file_path) {
                try {
                    const response = await fetch("{{ route('admin.upload_files.getFile') }}", {
                        method: 'POST',
                        headers: {
