                                ...
                                <tbody id="DataTableBody">
                                    @foreach ($Rejoins as $rejoin)
                                        <tr data-user-id="a" data-exchange-id="a">
                                            <td style="width: 45%;" class="encrypted-data">{{ $rejoin->user->name }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $rejoin->exchange->name }}
                                            </td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $rejoin->name }}</td>
                                            <td style="width: 45%;" class="encrypted-data">
                                                {{ $rejoin->phone->phone_number }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $rejoin->feedback }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $rejoin->amount }}</td>
                                            <td>{{ $rejoin->updated_at }}</td>
                                            <td style="width: 10%; text-align: center;">
                                                <div class="d-flex justify-content-center ">
                                                    <form method="POST" action="{{ route('admin.rejoin.delete') }}">
                                                        @csrf
                                                    ...
                                                    <button type="button" class="btn btn-info btn-sm edit-button mx-3"
                                                        data-id="{{ $rejoin->id }}" data-name="{{ $rejoin->name }}"
                                                        data-phone="{{ $rejoin->phone->phone_number }}"
                                                        data-feedback="{{ $rejoin->feedback }}"
                                                        data-amount="{{ $rejoin->amount }}">
                                                        Edit
                                                    </button>
                                                </div>
                                            </td>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header d-flex justify-content-between align-items-center " style="background:#acc301;">
                    <h5 class="modal-title" id="exportModalLabel" style="color:white">Export Rejoin</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">

                    <form id="reportForm" action="{{ route('export.rejoin') }}" method="POST">
                        @csrf
                        <div class="mb-3">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title text-white" id="editDemoSendModalLabel">Edit Walk Data</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            document.getElementById('editDemoSendForm').addEventListener('submit', function(event) {
                ...
                formData.append('task_name', 'rejoin');
