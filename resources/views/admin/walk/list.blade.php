                                ...
                                <tbody id="DataTableBody">
                                    @foreach ($Walks as $walk)
                                        <tr data-user-id="a" data-exchange-id="a">
                                            <td style="width: 45%;" class="encrypted-data">{{ $walk->user->name }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $walk->exchange->name }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $walk->name }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $walk->phone->phone_number ?? ''}}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $walk->feedback }}</td>
                                            <td style="width: 45%;" class="encrypted-data">{{ $walk->amount }}</td>
                                            <td >{{ $walk->updated_at }}</td>
                                            <td style="width: 10%; text-align: center;">
                                                ...
                                                <button type="button" class="btn btn-info btn-sm edit-button mx-3"
                                                    data-id="{{ $walk->id }}"
                                                    data-name="{{ $walk->name }}" data-phone="{{ $walk->phone->phone_number ?? ''}}"
                                                    data-feedback="{{ $walk->feedback }}"
                                                    data-amount="{{ $walk->amount }}">
                                                    Edit 
                                                </button>
                                            </div>
                                            </td>
...
<div class="modal fade" id="editDemoSendModal" tabindex="-1" aria-labelledby="editDemoSendModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.querySelectorAll('.edit-button').forEach(button => {
        button.addEventListener('click', function() {
            ...
            // Show the modal
            const editDemoSendModal = new bootstrap.Modal(document.getElementById('editDemoSendModal'));
    ...
    document.getElementById('editDemoSendForm').addEventListener('submit', function(event) {
        ...
        formData.append('task_name', 'walk');
