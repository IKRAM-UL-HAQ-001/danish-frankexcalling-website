                                ...
                                <tbody id="DataTableBody">
                                    @foreach ($Rejects as $reject)
                                        <tr data-user-id="a" data-exchange-id="a">
                                            <td class="encrypted-data">{{ $reject->name }}</td>
                                            <td class="encrypted-data">{{ $reject->phone->phone_number ?? ''}}</td>
                                            <td class="encrypted-data">{{ $reject->feedback }}</td>
                                            <td class="encrypted-data">{{ $reject->amount }}</td>
                                            <td class="">{{ $reject->created_at }}</td>
                                            <td>
                                                <button class="btn btn-primary btn-sm open-modal-btn" data-bs-toggle="modal"
                                                data-id = {{$reject->phone_id}}
                                                    data-bs-target="#addPhoneNumberModal" data-phone="{{ $reject->phone->phone_number  ?? ''}}">
                                                    Form
                                                </button>
                                            </td>
                                        </tr>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="reportForm" action="{{ route('export.reject') }}" method="POST">
                        @csrf
    ...
    <div class="modal fade" id="addPhoneNumberModal" tabindex="-1" aria-labelledby="addPhoneNumberModalLabel"
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="addPhoneForm">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control border px-3 customer_phone encrypted-data"
                                id="customer_phone" name="customer_phone" readonly>
                ...
                <div class="modal-footer">
                    <button type="button" id="demo_send_btn" class="btn btn-warning"
                        data-action="{{ route('customer_care.data_entry.post') }}">Demo Send</button>
                    <button type="button" id="reject_btn" class="btn btn-warning"
                        data-action="{{ route('customer_care.data_entry.post') }}">Reject</button>
                    <button type="button" id="rejoin_btn" class="btn btn-primary"
                        data-action="{{ route('customer_care.data_entry.post') }}">Rejoin</button>
                    <button type="button" id="refer_id_btn" class="btn btn-warning"
                        data-action="{{ route('customer_care.data_entry.post') }}">Refer ID</button>
                    <button type="button" id="new_id_btn" class="btn btn-success"
                        data-action="{{ route('customer_care.data_entry.post') }}">New Id</button>
                    <button type="button" id="follow_up_btn" class="btn btn-info"
                        data-action="{{ route('customer_care.data_entry.post') }}">Follow Up</button>
                    <button type="button" id="complaint_btn" class="btn btn-dark"
                        data-action="{{ route('customer_care.data_entry.post') }}">Complaint</button>
                    <button type="button" id="walk_btn" class="btn btn-primary"
                        data-action="{{ route('customer_care.data_entry.post') }}">Walk</button>
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                </div>
            </div>
        </div>
